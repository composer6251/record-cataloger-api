package com.recordcataloguer.recordcataloguer.util.discogs.validators;
...
public class DiscogsSearchResultValidator {
    ...
    public static List<Album> validateSearchResultsByRequestAndResults(List<Album> results, DiscogsSearchAlbumRequest albumRequest, List<DiscogsSearchAlbumRequest> originals) {
        ...
        List<Album> resultsWithoutDuplicateTitles;
        resultsWithoutDuplicateTitles = filterOutResponseDuplicates(results);
        ...
        if(resultsWithoutDuplicateTitles.size() == 1 &&
                (resultsWithoutDuplicateTitles.get(0).getCatno().equalsIgnoreCase(albumRequest.getCatNo()) ||
                        resultsWithoutDuplicateTitles.get(0).getCatno().equalsIgnoreCase(albumRequest.getTitle()))) {
            return resultsWithoutDuplicateTitles;
        }
        ...
        List<Album> finalResultsWithoutDuplicateTitles = resultsWithoutDuplicateTitles;
        titleFilteredAlbums = originals.stream()
                .flatMap(o -> finalResultsWithoutDuplicateTitles.stream()
    ...
    public static List<Album> filterOutResponseDuplicates(List<Album> searchResults) {
        if(searchResults == null || searchResults.isEmpty()) {
            log.info("No albums found for user or user not authenticated.");
            return null;
        }
